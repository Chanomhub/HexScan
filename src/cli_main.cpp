...
void printUsage() {
    ...
    std::cout << "  iscan (interactive scan: new/next/count/list/quit)" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    if (cmd == "scan") {
        ...
        // Interactive scan mode: keeps scanner alive for new → next → next flow
        // Commands read from stdin (one per line):
        //   new <type> <value> [scan_type_int]   — first scan
        //   next <value> [scan_type_int]          — next scan (narrow down)
        //   count                                 — print current result count
        //   list [max]                            — list found addresses
        //   reset                                 — reset scanner
        //   quit                                  — exit
        
        Scanner scanner("IScan");
        scanner.getRegions().mustHavePerms = (RegionPerms)0;
        scanner.getRegions().mustNotHavePerms = (RegionPerms)0;
        ...
        auto setValueForScan = [&](const std::string& valStr) {
            if (typeStr == "string") {
                std::vector<uint8_t> bytes(valStr.begin(), valStr.end());
                scanner.setValue(bytes);
            } else if (typeStr == "aob") {
                auto aob = ParseAOBString(valStr);
                if (!aob.success) { std::cerr << "Invalid AOB: " << aob.errorMessage << std::endl; return false; }
                scanner.setValue(aob.bytes);
                scanner.setValueMask(aob.mask);
                scanner.setFastScanOffset(1);
            } else if (typeStr == "f32" || typeStr == "f64" || typeStr == "all") {
                double dval = std::stod(valStr);
                std::vector<uint8_t> bytes(8);
                std::memcpy(bytes.data(), &dval, 8);
                scanner.setValue(bytes);
        ...
        while (std::getline(std::cin, line)) {
            ...
            if (subcmd == "quit" || subcmd == "q") break;
            
            if (subcmd == "new") {
                std::string valStr;
                int scanTypeInt = 0;
                iss >> typeStr >> valStr;
                if (iss) iss >> scanTypeInt;
                
                if (typeStr.empty() || valStr.empty()) {
                    std::cerr << "Usage: new <type> <value> [scan_type]" << std::endl;
                    continue;
                }
                ...
                else if (typeStr == "string") { vt = {string}; vt.stringLength = valStr.length(); }
                else if (typeStr == "aob") vt = {byteArray};
                else if (typeStr == "all") vt = {all};
                else { std::cerr << "Unknown type: " << typeStr << std::endl; continue; }
                ...
                scanner.setValueType(vt);
                scanner.setScanType((ScanType)scanTypeInt);
                if (!setValueForScan(valStr)) continue;
                hasType = true;
                
                scanner.newScan();
                waitForScan();
                
                auto addrs = scanner.getAddresses();
                std::cout << "SCAN_RESULT " << addrs.size() << std::endl;
                
            } else if (subcmd == "next") {
                if (!hasType) { std::cerr << "Do 'new' first." << std::endl; continue; }
                ...
                if (valStr.empty()) {
                    // For scans that don't need a value (Changed, Unchanged, Increased, Decreased)
                    valStr = "0";
                }
                ...
                auto addrs = scanner.getAddresses();
                std::cout << "COUNT " << addrs.size() << std::endl;
                
            } else if (subcmd == "list") {
                int maxPrint = 100;
                iss >> maxPrint;
                auto addrs = scanner.getAddresses();
                std::cout << "LIST " << addrs.size() << std::endl;
                int c = 0;
                for (auto a : addrs) {
                    if (c++ >= maxPrint) break;
                ...
                std::cerr << "Unknown iscan command: " << subcmd << std::endl;
            }
        }
        std::cout << "ISCAN_EXIT" << std::endl;
    }
    else if (cmd == "disasm") {
